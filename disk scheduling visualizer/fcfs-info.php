...
<body>
  ...
  <div class="py-3 mb-3">
    <div class="container">
      ...
      <br>
      <p>FCFS (First Come First Serve) is simplest disk scheduling algorithm. Each requests are taken/executed in the sequence of their arrival in the disk queue. As a result, there is no starvation in this algorithm. However, it isn't fast compared to other algorithms.</p>
      <br>

      <h3> Advantages:</h3>
      <ul>
        <li>There is no starvation in FCFS.</li>
        <li>FCFS is simple and straight forward approach which means there are no indefinite delays.</li>
      ...
      <h3> Disadvantages:</h3>
      <ul>
        <li>FCFS isn't considered an efficient and optimized approach.</li>
        <li>FCFS can be time consuming.</li>
      </ul>

      <h3> Example: </h3>
      <ul>
        <li>Suppose a disk contains 200 tracks (0-199) and the request queue contains track no: 176, 79, 34, 60, 92, 11, 41, 114.</li>
        <li>The current position of the read/write head is 50. Now we have to calculate the total number of track movements of read/write head using FCFS scheduling.</li>
        <li>As mentioned in the following example, the disk contains 200 tracks, so we take a track line between 0 to 199.</li>
        <li>The current position of the read/write head is 50. So, we start from 50, then move read/write head in the FCFS order. When all the requests are addressed, then we calculate a total number of cylinders moved by the head.</li>
        <br>
        <img src="images/fcfs-eg-graph.jpg" alt="graph of example">
        <br><br>
        <li>Total Number of cylinders moved by the head = (176-50)+(176-79)+(79-34)+(60-34)+(92-60)+(92-11)+(41-11)+(114-41) = 510</li>
      </ul>
      ...
      <a class="simulate" href="FCFS/fcfs.php">Visualize</a>
    </div>
  </div>
